...
STATUS_TEXT = (
    "l/h:±1d, j/k:±1w, f/b:±1m, n/p:±1y, t:today, r:reload, "
...
class TextBox(object):
    ...
    def __init__(
        self, scr, x=0, y=0, width=0, height=0, text="", align=ALIGN_LEFT
    ):
        self.scr = scr
        self.x = x
        self.y = y
    ...
    def render(self, highlight=False, offset=0):
        ...
        for i in range(min(self.content_height, self.height)):
            self._addstr(self.x, self.y + i, lines[i + self.offset], highlight)
...
class ObdaRemind(object):
    def __init__(self, scr):
        ...
        self.boxes = {
            ...
            "weekdays": [
                TextBox(scr, text=w, align=TextBox.ALIGN_CENTER)
                for w in WEEKDAYS
            ],
    ...
    def redraw(self):
        ...
        for i in range(6):
            ...
            for j in range(7):
                ...
                self.scr.vline(
                    v_offset + 1, h_offset, curses.ACS_SBSB, datebox_height
                )
        ...
        if status_height >= 0:
            ...
            for i in range(7):
                self.scr.hline(
                    v_offset, i * (datebox_width + 1) + 1, curses.ACS_SSBS, 1
                )
            self.scr.vline(v_offset, notes_border, curses.ACS_SBSS, 1)
        # Relocate the text boxes.
        self.boxes["header"].relocate(0, 0, notes_border, 1)
        self.boxes["notes"].relocate(
            notes_border + 1, 0, notes_width, notes_height
        )
        self.boxes["status"].relocate(
            0, scr_height - status_height, notes_border, status_height
        )
    ...
    def set_date(self, new_date):
        ...
        first_sunday = first_date - datetime.timedelta(
            days=(first_weekday + 1) % 7
        )
        first_monday = first_sunday + datetime.timedelta(1)
        # Load a new month?
        if (
            not self.selected
            or new_date.month != self.selected.month
            or new_date.year != self.selected.year
        ):
    ...
    def jump_months(self, months):
        ...
        # Erm, get the last day of the month instead.
            ...
            new_date = self.selected.replace(
                month=month + 1, day=1, year=year
